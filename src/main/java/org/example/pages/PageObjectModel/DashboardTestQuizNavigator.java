...
public class DashboardTestQuizNavigator extends CommonToAllPage {
    ...
    private By Dashboard_Popup =
            By.xpath("//h5[contains(text(), 'Hostel Admission')]");

    private By Modal_Dismiss =
            By.xpath("//a[@data-dismiss=\"modal\"]");

    private By giveTestButton =
            By.xpath("//input[@onclick=\"callAlert(4141,'Objective')\"]");

    private By Test_Quiz =
            By.xpath("//p[contains(text(),'Test/Quiz')]");
    ...
    public void navigateToTestQuizSafely() {
        ...
        try {
            WebElement popup =
                    wait.until(ExpectedConditions.presenceOfElementLocated(Dashboard_Popup));

            WebElement closeBtn =
                    wait.until(ExpectedConditions.elementToBeClickable(Modal_Dismiss));
        ...
        WebElement testQuiz =
                wait.until(ExpectedConditions.visibilityOfElementLocated(Test_Quiz));

        ((JavascriptExecutor) getDriver())
                .executeScript("arguments[0].scrollIntoView({block:'center'});", testQuiz);

        ((JavascriptExecutor) getDriver())
                .executeScript("arguments[0].click();", testQuiz);
